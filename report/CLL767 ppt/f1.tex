\documentclass[aspectratio=169]{beamer}

% --- THEME AND STYLING ---
\usetheme{Madrid}
\useoutertheme{default}
\useinnertheme{rectangles}
\setbeamertemplate{navigation symbols}{}

% --- CUSTOM COLOR PALETTE ---
\definecolor{SkyBlue}{RGB}{135, 206, 235}
\definecolor{DarkNavy}{RGB}{0, 0, 139}
\definecolor{LightBlueBg}{RGB}{240, 248, 255}
\definecolor{AccentBlue}{RGB}{70, 130, 180}
\definecolor{DarkGreyText}{RGB}{50, 50, 50}
\definecolor{HighlightGreen}{RGB}{34, 139, 34}
\definecolor{AlertRed}{RGB}{220, 20, 60}

\setbeamercolor{palette primary}{bg=SkyBlue,fg=DarkNavy}
\setbeamercolor{palette secondary}{bg=DarkNavy,fg=white}
\setbeamercolor{palette tertiary}{bg=AccentBlue,fg=white}
\setbeamercolor{palette quaternary}{bg=SkyBlue,fg=DarkNavy}
\setbeamercolor{structure}{fg=DarkNavy}
\setbeamercolor{section in toc}{fg=DarkNavy}
\setbeamercolor{frametitle}{bg=SkyBlue,fg=DarkNavy}
\setbeamercolor{block title}{bg=DarkNavy,fg=white}
\setbeamercolor{block body}{bg=LightBlueBg,fg=DarkGreyText}
\setbeamercolor{block title alerted}{bg=AlertRed,fg=white}
\setbeamercolor{block body alerted}{bg=LightBlueBg,fg=DarkGreyText}
\setbeamercolor{block title example}{bg=HighlightGreen,fg=white}
\setbeamercolor{block body example}{bg=LightBlueBg,fg=DarkGreyText}
\setbeamertemplate{footline}[frame number]
\setbeamercolor{footline}{fg=DarkNavy}
\setbeamertemplate{blocks}[rounded][shadow=false]

% --- PACKAGES ---
\usepackage[utf8]{inputenc}
\usepackage{amsmath,amssymb}
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage{array}
\usepackage{multirow}
\usepackage{tikz}
\usetikzlibrary{shapes,arrows,positioning,calc,patterns,decorations.pathmorphing}
\usepackage{siunitx}
\usepackage{textcomp}

% --- BIBLIOGRAPHY ---
\usepackage[numbers,sort&compress]{natbib}
\bibliographystyle{unsrt}

% --- TITLE PAGE ---
\title[PVDF in Energy Storage]{Polyvinylidene Fluoride (PVDF) in Energy Storage}
\subtitle{From Incumbent Binder to Next-Generation Functional Material}
\author{}
\institute{Department of Chemical Engineering\\
\vspace{0.2cm}
\small Course: CLL767 - Structures \& Properties of Polymers\\
\small Instructor: Prof. Sudip K. Pattanaik}
\date{\today}

\begin{document}

% --- TITLE SLIDE ---
\begin{frame}[plain]
    \titlepage
    \vspace{-0.5cm}
    \begin{center}
        \small Presented by: Yash Sakhare (2022CH71496), Sudarshan Oraon (2022CH71511)
    \end{center}
\end{frame}

% --- OUTLINE ---
\begin{frame}{Outline}
    \small
    \tableofcontents[hideallsubsections]
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Introduction \& Polymer Science}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{What is Polyvinylidene Fluoride (PVDF)?}
    \begin{block}{A Critical Fluoropolymer}
        \begin{itemize}
            \item High-performance, semi-crystalline thermoplastic fluoropolymer
            \item Synthesized from vinylidene fluoride (VDF): \textbf{-[CH$_2$-CF$_2$]-}$_n$
            \item \textbf{Key feature:} Highly polar C-F bonds $\rightarrow$ large dipole moment
            \item Exceptional chemical resistance, thermal stability ($>$400\si{\celsius}), UV resistance\cite{ref13}
        \end{itemize}
    \end{block}
    
    \begin{alertblock}{Critical Role in Energy Storage}
        \textbf{Dual Function in Lithium-Ion Batteries:}
        \begin{enumerate}
            \item Industry-standard \textbf{polymeric binder} for electrodes
            \item High-performance \textbf{battery separator} material
        \end{enumerate}
    \end{alertblock}
    
    \begin{exampleblock}{Molecular Weight Engineering}
        \footnotesize Battery grades: $M_w$ $\sim$600,000 g/mol; Ultra-high grades: $>$1,000,000 g/mol\cite{ref9,ref1}
    \end{exampleblock}
\end{frame}

\begin{frame}{The Key to PVDF: Crystalline Polymorphism}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{$\alpha$-Phase (Alpha)}
                \begin{itemize}\small
                    \item Most common \& thermodynamically stable
                    \item Forms when crystallized from melt
                    \item \textbf{Conformation:} TGTG' (Trans-Gauche)
                    \item Dipoles cancel $\rightarrow$ \textbf{non-polar}
                    \item Used in standard processing
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{$\beta$-Phase (Beta)}
                \begin{itemize}\small
                    \item Most technologically significant
                    \item Forms via stretching or solution-casting
                    \item \textbf{Conformation:} TTTT (all-trans)
                    \item All C-F dipoles align $\rightarrow$ \textbf{highly polar}
                    \item Piezoelectric \& ferroelectric properties
                \end{itemize}
            \end{alertblock}
        \end{column}
    \end{columns}
    
    \vspace{0.3cm}
    \begin{exampleblock}{Performance Impact}
        $\beta$-dominant PVDF binder: \textbf{185 mAh/g capacity \& 98\% retention} vs. $\alpha$-phase: 71.6\% retention after 50 cycles\cite{ref1}
    \end{exampleblock}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Quantified Properties for Batteries}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{Key Thermal Properties}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Glass Transition ($T_g$)}
                \begin{itemize}
                    \item \textbf{Value:} -35 to -40\si{\celsius}\cite{ref13}
                    \item Far below battery operating temp
                    \item Amorphous regions stay \textbf{flexible}
                    \item Accommodates volume changes without cracking
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Melting Temperature ($T_m$)}
                \begin{itemize}
                    \item \textbf{Value:} 165-175\si{\celsius}\cite{ref13}
                    \item \textbf{Critical safety advantage!}
                    \item PE/PP separators melt at $\sim$130\si{\celsius}
                    \item PVDF's higher $T_m$ prevents catastrophic shorts during thermal runaway
                \end{itemize}
            \end{alertblock}
        \end{column}
    \end{columns}
    
    \vspace{0.3cm}
    \begin{center}
        \begin{tikzpicture}[scale=0.8]
            \draw[<->, thick] (0,3) node[left] {Temp} -- (0,0) -- (8,0) node[below] {Time};
            \draw[thick, blue] (0,0.5) -- (2,0.5) -- (2,1) -- (4,1) -- (4,2.5) node[above right] {PVDF: 170\si{\celsius}};
            \draw[thick, red] (0,0.3) -- (2,0.3) -- (2,0.7) -- (3.5,0.7) -- (3.5,1.8) node[above right] {PE: 130\si{\celsius}};
            \draw[dashed] (0,2) -- (8,2) node[right, font=\small] {Danger Zone};
        \end{tikzpicture}
    \end{center}
\end{frame}

\begin{frame}{The Master Property: Crystallinity}
    \begin{block}{Percent Crystallinity (\%)}
        \begin{itemize}
            \item Typical PVDF: 50-70\% crystalline\cite{ref13}
            \item Battery grades: engineered from 14\% to 32\%\cite{ref9}
            \item \textbf{Must balance:} strength, adhesion, electrolyte uptake
        \end{itemize}
    \end{block}
    
    \begin{alertblock}{Decomposition Temperature}
        \begin{itemize}
            \item \textbf{Onset:} $>$400\si{\celsius} (TGA data)\cite{ref13}
            \item Electrodes dried at $\sim$130\si{\celsius} (NMP evaporation)\cite{ref9}
            \item \textbf{Wide processing window} ensures polymer integrity
        \end{itemize}
    \end{alertblock}
    
    \begin{exampleblock}{Crystallization Kinetics (Avrami Model)}
        \footnotesize $1 - V_c(t) = \exp(-kt^n)$ with $n = 1.5-2.4$; $T_m^0 = 182$\si{\celsius}
    \end{exampleblock}
\end{frame}

\begin{frame}{The Crystallinity Trade-Off: Data}
    \begin{table}
        \caption{Influence of PVDF Crystallinity on Electrode Performance\cite{ref1,ref9}}
        \centering\small
        \begin{tabular}{lccl}
            \toprule
            \textbf{Property} & \textbf{PVDF 1 (Low)} & \textbf{PVDF 2 (High)} & \textbf{Key Insight} \\
            \midrule
            \% Crystallinity & 14\% & \textbf{32\%} & Input variable \\
            \textbf{Adhesion} & 1.30 N/cm & \textbf{11.42 N/cm} & \textcolor{HighlightGreen}{\textbf{9$\times$ stronger!}} \\
            Electrolyte Uptake & \textbf{18.88\%} & 11.3\% & Lower = better \\
            \midrule
            \textbf{LFP Capacity} & 136 mAh/g & \textbf{146 mAh/g} & Higher capacity \\
            \textbf{Retention (500c)} & 64\% & \textbf{82\%} & \textcolor{HighlightGreen}{\textbf{18\% better}} \\
            \bottomrule
        \end{tabular}
    \end{table}
    
    \vspace{0.2cm}
    \begin{exampleblock}{Key Finding}
        High crystallinity $\rightarrow$ strong adhesion \& controlled swelling $\rightarrow$ \textbf{superior long-term performance}\cite{ref1,ref16}
    \end{exampleblock}
\end{frame}

\begin{frame}{The Crystallinity Trade-Off: Analysis}
    \begin{block}{Crystallinity $\rightarrow$ Adhesion}
        \small High-crystallinity binder: \textbf{$\sim$9$\times$ stronger adhesion} from increased intermolecular interactions
    \end{block}
    
    \begin{block}{Crystallinity $\rightarrow$ Swelling}
        \small Low-crystallinity: larger amorphous volume $\rightarrow$ only region for electrolyte $\rightarrow$ \textbf{higher swelling}
    \end{block}
    
    \begin{alertblock}{The Non-Obvious Conclusion}
        \small \textbf{Excessive swelling is detrimental!}
        \begin{itemize}\small
            \item High swelling destroys carbon distribution in electrode
            \item Increases internal resistance $\rightarrow$ rapid capacity fade
            \item \textbf{Goal:} Optimize crystallinity for adhesion + controlled swelling
        \end{itemize}
    \end{alertblock}
\end{frame}

\begin{frame}{Electrochemical \& Chemical Properties}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Electrochemical Stability}
                \begin{itemize}\small
                    \item \textbf{The Gold Standard: 0-5 V}\cite{ref13}
                    \item Stable at graphite anode ($\sim$0.1 V)
                    \item Stable at high-voltage cathodes ($>$4.3 V)
                    \item One of few polymers with wide window
                \end{itemize}
            \end{block}
            
            \begin{block}{Dielectric Constant}
                \begin{itemize}\small
                    \item \textbf{Value:} $\epsilon$ = 8.4 to 12\cite{ref13}
                    \item High polarity ensures easy wetting
                    \item Aids Li-salt dissociation
                \item Higher ionic conductivity
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{The NMP Problem}
                \textbf{Limited Solubility:} Only dissolves in polar aprotic solvents
                \begin{itemize}\small
                    \item \textbf{NMP} (N-methyl-2-pyrrolidone)
                    \item DMF, DMAc, DMSO
                \end{itemize}
                
                \vspace{0.1cm}
                \textbf{Why NMP is standard:}\cite{ref13}
                \begin{itemize}\small
                    \item High flash point ($\sim$91\si{\celsius})
                    \item Low vapor pressure
                    \item Up to 99\% recyclable
                \end{itemize}
                
                \vspace{0.1cm}
                \textcolor{AlertRed}{\textbf{BUT:}} Classified as \textbf{SVHC} \& reproductive hazard!
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{The NMP Challenge: A Sustainability Crisis}
    \begin{block}{Why NMP is a Major Liability}
        \begin{itemize}\small
            \item \textbf{Toxicity:} Known developmental \& reproductive toxin
            \item \textbf{Environmental:} VOC with strict EU REACH regulations
            \item \textbf{Cost:} Expensive + high-energy recovery systems
        \end{itemize}
    \end{block}
    
    \begin{exampleblock}{Solution: Hansen Solubility Parameters (HSP)}
        \small \textbf{Predictive framework for finding green solvents!}
        
        \vspace{0.05cm}
        HSP decomposes cohesive energy: dispersion ($\delta_D$), polar ($\delta_P$), H-bonding ($\delta_H$)
        
        \vspace{0.05cm}
        \textbf{Key Metric: Relative Energy Difference (RED)}
        \begin{itemize}\small
            \item RED $<$ 1.0: High affinity, dissolution likely
            \item RED $>$ 1.0: Low affinity, won't dissolve
        \end{itemize}
    \end{exampleblock}
\end{frame}

\begin{frame}{Green Solvent Alternatives: Computational Screening}
    \begin{block}{HSP-Based Screening of 224 Bio-Derived Solvents}
        \small \textbf{Validated PVDF model predicts solubility before costly trials!}
    \end{block}
    
    \vspace{-0.2cm}
    \begin{table}
        \caption{RED Values for PVDF Solvents (HSP-based screening)\cite{ref13}}
        \centering\footnotesize
        \begin{tabular}{lcc}
            \toprule
            \textbf{Solvent} & \textbf{RED} & \textbf{Assessment} \\
            \midrule
            \textbf{NMP (Reference)} & \textcolor{HighlightGreen}{\textbf{0.50}} & Excellent \\
            \textbf{Cyrene (Bio)} & \textcolor{HighlightGreen}{\textbf{0.78}} & Good \\
            \textbf{$\gamma$-Valerolactone} & \textcolor{HighlightGreen}{\textbf{0.80}} & Good \\
            \textbf{DMSO} & \textcolor{HighlightGreen}{\textbf{0.88}} & Good \\
            \bottomrule
        \end{tabular}
    \end{table}
    
    \vspace{-0.2cm}
    \begin{alertblock}{Key Insight}
        \small \textbf{Cyrene \& GVL are viable NMP replacements}
        \begin{itemize}\footnotesize
            \item Thermodynamically capable of dissolving PVDF
            \item Bio-derived, non-toxic sources
            \item Identify solutions before lab testing
        \end{itemize}
    \end{alertblock}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Advanced Characterization Methods}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{How Properties Are Measured}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Phase \& Crystallinity}
                \textbf{FTIR:} Identifies crystal phase
                \begin{itemize}\small
                    \item Each phase has unique vibrational fingerprint\cite{ref13}
                    \item $\beta$-phase: sharp peak at 838 cm$^{-1}$\cite{ref13}
                \end{itemize}
                
                \textbf{XRD:} Quantifies crystallinity
                \begin{itemize}\small
                    \item Sharp peaks = crystalline regions
                    \item Broad halo = amorphous regions
                    \item Integrate areas to calculate \%
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{block}{Thermal Analysis}
                \textbf{DSC:} Measures heat flow
                \begin{itemize}\small
                    \item $T_g$ appears as small step
                    \item $T_m$ appears as large peak
                    \item Peak area $\rightarrow$ \% crystallinity
                \end{itemize}
                
                \textbf{TGA:} Measures mass vs. temp
                \begin{itemize}\small
                    \item Stable plateau up to $>$400\si{\celsius}
                    \item Sharp drop = decomposition
                    \item Confirms wide processing window
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Advanced Characterization Techniques}
    \begin{block}{Slurry Rheology (Rotational Rheometer)}
        \textbf{Critical for processability!} Measures flow behavior:
        \begin{enumerate}
            \item \textbf{Shear-Thinning:} Viscosity $\downarrow$ at high shear (easy coating)
            \item \textbf{Viscoelastic Gel:} $G' > G''$ at rest (prevents settling)
        \end{enumerate}
    \end{block}
    
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Mechanical Analysis}
                \textbf{Peel Test:} Quantifies adhesion\cite{ref16}
                \begin{itemize}\small
                    \item 90° or 180° peel configuration
                    \item PVDF: 8.7-48 N/m
                \end{itemize}
                \textbf{DMA:} Dynamic properties
                \begin{itemize}\small
                    \item Storage modulus ($G'$) \& loss modulus ($G''$)
                    \item Accurate $T_g$ determination
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{block}{Electrochemical Analysis}
                	extbf{EIS:} Impedance spectroscopy\cite{ref13}
                \begin{itemize}\small
                    \item Measures ionic conductivity
                    \item Identifies charge-transfer resistance
                \end{itemize}
                \textbf{Fracture Mechanics:}
                \begin{itemize}\small
                    \item Essential Work of Fracture (EWF)
                    \item Ductile-brittle transition analysis
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Applications: Binders \& Separators}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{Application I: The Electrode Binder}
    \begin{block}{Primary Role: Cohesion \& Adhesion}
        Despite only 2-5\% of electrode weight, function is \textbf{non-negotiable}:
        \begin{itemize}
            \item \textbf{Cohesion:} Binds active material + conductive carbon together
            \item \textbf{Adhesion:} Adheres composite to current collector (Al/Cu foil)
        \end{itemize}
    \end{block}
    
    \begin{alertblock}{PVDF's Adhesion Mechanism: Physical Only}
        \begin{enumerate}\small
            \item \textbf{Van der Waals Forces:} Weak induced-dipole interactions
            \item \textbf{Mechanical Interlocking:} Polymer chains entangle in surface roughness
            \item \textbf{No Chemical Bonding:} Lacks -COOH/-OH groups for covalent bonds
        \end{enumerate}
    \end{alertblock}
    
    \begin{block}{Electrochemical Requirements}
        \begin{enumerate}
            \item \textbf{Stability:} Inert in 0-5 V window
            \item \textbf{Chemical Inertness:} No reaction with electrolyte
            \item \textbf{Controlled Swelling:} Absorb electrolyte for ion transport
        \end{enumerate}
    \end{block}
\end{frame}

\begin{frame}{The Silicon Anode Problem}
    \begin{alertblock}{Why Silicon?}
        \begin{itemize}
            \item Silicon offers \textbf{10$\times$ higher theoretical capacity} than graphite\cite{ref14}
            \item Most promising next-generation anode material
        \end{itemize}
    \end{alertblock}
    
    \begin{alertblock}{The Catch: Massive Volume Expansion}
        \begin{itemize}
            \item \textbf{$>$300\% volume expansion} during charging (lithiation)\cite{ref14}
            \item Graphite: only $\sim$10\% expansion
        \end{itemize}
    \end{alertblock}
    
    \begin{block}{PVDF's Fundamental Failure}
        \begin{itemize}
            \item High modulus makes it \textbf{too stiff \& brittle}\cite{ref5}
            \item Rigid PVDF shell \textbf{cannot stretch} $\rightarrow$ \textbf{cracks}
            \item Leads to electrode pulverization, loss of electrical contact
            \item \textbf{Rapid, irreversible battery failure}
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}{Application II: High-Performance Separator}
    \begin{block}{Dual Functions}
        \begin{enumerate}
            \item \textbf{Physical Separation:} Prevents anode-cathode contact (short circuit)
            \item \textbf{Ionic Transport:} Microporous structure allows Li$^+$ ions to pass
        \end{enumerate}
    \end{block}
    
    \begin{table}
        \caption{PVDF vs. Polyolefin Separators\cite{ref13}}
        \centering\footnotesize
        \begin{tabular}{llll}
            \toprule
            \textbf{Property} & \textbf{PE/PP} & \textbf{PVDF} & \textbf{Benefit} \\
            \midrule
            $T_m$ & $\sim$130-165\si{\celsius} & \textbf{$\sim$170\si{\celsius}} & \textcolor{HighlightGreen}{Higher safety} \\
            Wettability & Poor (non-polar) & \textbf{Excellent} & \textcolor{HighlightGreen}{Lower resistance} \\
            Electrolyte Uptake & 48-65\% & \textbf{92-95\%} & \textcolor{HighlightGreen}{Higher capacity} \\
            Fabrication & Stretching & \textbf{Electrospinning} & \textcolor{HighlightGreen}{Tunable pores} \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Sustainability Crisis \& Alternatives}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{The Sustainability Crisis: NMP Problem}
    \begin{alertblock}{Toxicity \& Regulation}
        \small
        \begin{itemize}
            \item NMP classified as \textbf{SVHC}
            \item Known \textbf{reproductive toxicant}
            \item Heavily restricted by EU REACH
            \item High compliance costs
        \end{itemize}
    \end{alertblock}
    
    \vspace{-0.15cm}
    \begin{block}{The "Aqueous Processing Revolution"}
        \small \textbf{Cost Comparison:}
        \begin{itemize}\small
            \item Water: \textbf{$<$\$0.02/L}
            \item NMP: \textbf{\$1.00-\$3.00/L} \quad \textcolor{AlertRed}{50-150$\times$ more!}
        \end{itemize}
        
        \vspace{0.1cm}
        \textbf{Additional NMP Costs:}
        \begin{itemize}\small
            \item High-CAPEX solvent recovery
            \item Energy-intensive drying
            \item VOC compliance \& regulations
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}{End-of-Life Recycling Crisis}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Pyrometallurgy (Burning)}
                \small \textbf{Process:} High-temp incineration (600-1600\si{\celsius})
                
                \vspace{0.1cm}
                \textbf{The Problem:}
                \begin{itemize}\small
                    \item PVDF decomposes $\rightarrow$ \textbf{HF gas}\cite{ref39}
                    \item Hydrogen fluoride: corrosive \& toxic
                    \item Requires costly acid scrubbers
                    \item Deactivates cathode materials
                \end{itemize}
            \end{alertblock}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{block}{Hydrometallurgy (Leaching)}
                \small \textbf{Process:} Acids dissolve \& separate metals
                
                \vspace{0.1cm}
                \textbf{The Problem:}
                \begin{itemize}\small
                    \item PVDF's resistance = \textbf{doesn't dissolve}\cite{ref39}
                    \item Remains as solid contaminant
                    \item Complicates metal purification
                \end{itemize}
            \end{block}
            
            \begin{exampleblock}{Future Solution: SC-CO$_2$ Extraction}
                \footnotesize \textbf{Supercritical CO$_2$ + DMSO co-solvent:}
                \begin{itemize}\footnotesize
                    \item Liquid density, gas diffusivity
                    \item Dissolves PVDF, recovers materials
                    \item No HF emission, green process
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Alternative Binders: Challenging Assumptions}
    \begin{block}{Common Misconception: PVDF Has Best Adhesion}
        \textbf{Data contradicts this!} Aqueous binders show \textbf{stronger adhesion}
    \end{block}
    
    \begin{table}
        \caption{Quantitative Binder Comparison~\cite{ref16,ref18}}
        \centering\footnotesize
        \begin{tabular}{lcccc}
            \toprule
            \textbf{Property} & \textbf{PVDF} & \textbf{PAA} & \textbf{CMC} & \textbf{Alginate} \\
            \midrule
            Solvent & \textcolor{AlertRed}{NMP} & Water & Water & Water \\
            Adhesion (N/cm) & $\sim$1.0 & $\sim$1.5 & 1.1-1.7 & \textbf{$\sim$2.0} \\
            Modulus (MPa) & 600-4000 & \textbf{$\sim$8000} & 1000 & 119-500 \\
            $T_g$ & -35\si{\celsius} & 106\si{\celsius} & 186\si{\celsius} & --- \\
            \bottomrule
        \end{tabular}
    \end{table}
    
    \begin{exampleblock}{The Reason: Hydrogen Bonding}
        \small Aqueous binders (PAA, CMC) have \textbf{-COOH/OH} groups $\rightarrow$ strong H-bonds with oxide layers. PVDF uses weaker van der Waals forces.
    \end{exampleblock}
\end{frame}

\begin{frame}{The Silicon Anode Solution}
    \begin{alertblock}{PVDF Fails for Silicon}
        \small Too stiff \& brittle to accommodate $>$300\% volume expansion
    \end{alertblock}
    
    \begin{exampleblock}{PAA: The Enabling Binder}\cite{ref16}
        \small \textbf{Polyacrylic Acid (PAA) is the binder of choice for Si-anodes:}
        \begin{itemize}\small
            \item Unique: \textbf{high stiffness} ($\sim$8000 MPa) + \textbf{elasticity}\cite{ref16}
            \item Can withstand \& accommodate extreme stresses
            \item Higher carboxylic acid groups $\rightarrow$ stronger Si bonds
            \item Lower swelling (7.49\%) vs. SBR (9.56\%)
        \end{itemize}
    \end{exampleblock}
    
    \begin{block}{The Aqueous Processing Challenge}
        \small \textbf{pH Control is Critical:}
        \begin{itemize}\small
            \item Alkaline slurries (pH $\sim$12) corrode Al collector
            \item Solution: Add acid to titrate pH to 9-10
        \end{itemize}
    \end{block}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Future of PVDF \& Market Analysis}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{The Future: Evolving PVDF's Role}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Gel-Polymer Electrolytes}\cite{ref21,ref13}
                \textbf{PVDF-HFP copolymers:}
                \begin{itemize}\small
                    \item Absorb liquid electrolyte into polymer matrix
                    \item Combine safety of solid with conductivity of liquid
                    \item Higher ionic conductivity than pure solid
                    \item Improved flexibility \& processability
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{block}{Composite Solid-State Electrolytes}
                \textbf{Problem:} Ceramic SSEs are brittle, poor electrode contact
                \begin{itemize}\small
                    \item PVDF as polymer binder phase
                    \item Disperses ceramic Li-conductors
                    \item Improves mechanical integrity
                    \item Enhances electrode-electrolyte contact
                \end{itemize}
            \end{block}
            
            \begin{exampleblock}{Biocompatibility}
                \footnotesize \textbf{FDA-approved:} Medical membranes, high-purity filtration
            \end{exampleblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Sodium-Ion Battery Incompatibility}
    \begin{alertblock}{PVDF Fails in Sodium Systems}
        \textbf{Fundamental electrochemical instability} in Na-based systems
    \end{alertblock}
    
    \begin{block}{The Failure Mechanism}
        \begin{enumerate}
            \item During sodiation, PVDF undergoes chemical decomposition
            \item Produces fluoride ions (F$^-$)
            \item F$^-$ + Na $\rightarrow$ forms stable, insulating \textbf{NaF layer}
            \item Resistive layer destroys binding ability
            \item High resistance $\rightarrow$ rapid cell failure
        \end{enumerate}
    \end{block}
    
    \begin{exampleblock}{Conclusion}
        PVDF's LIB success \textbf{does not translate to SIBs}. CMC/PAA required for Na-ion batteries.
    \end{exampleblock}
\end{frame}

\begin{frame}{Market \& Techno-Economic Analysis}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Market Size \& Growth}
                \small \textbf{Driven by EV \& grid storage:}
                \begin{itemize}\footnotesize
                    \item \textbf{2024:} USD 1,800.75 million
                    \item \textbf{2032:} USD 3,605.40 million
                    \item \textbf{CAGR:} 8.82\% (2025-2032)
                \end{itemize}
            \end{block}
            
            \begin{block}{Key Manufacturers}
                \footnotesize \textbf{Global supply chain:}\cite{ref9}
                \begin{itemize}\footnotesize
                    \item Arkema (France): Kynar®
                    \item Solvay (Belgium): Solef®
                    \item Kureha (Japan): Battery grades
                    \item Dongyue Group (China)
                    \item 3M, AGC Chemicals
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{The Real Cost Problem}
                \small \textbf{Polymer price:} \$7-26/kg\cite{ref9}
                \begin{enumerate}\footnotesize
                    \item \textbf{NMP Solvent:} \$1-3/L (50-150$\times$ water!)\cite{ref13}
                    \item \textbf{Solvent Recovery:} High CAPEX equipment
                    \item \textbf{Drying:} Energy-intensive (130\si{\celsius})\cite{ref9}
                    \item \textbf{VOC Compliance:} Regulatory costs
                    \item \textbf{Recycling:} HF scrubbing systems\cite{ref39}
                \end{enumerate}
            \end{alertblock}
            
            \vspace{0.05cm}
            \begin{center}
                \begin{tikzpicture}[scale=0.55]
                    \draw[fill=AlertRed] (0,0) rectangle (1.5,2.5) node[midway,white,font=\tiny] {NMP};
                    \draw[fill=HighlightGreen] (2,0) rectangle (3.5,0.3) node[midway,white,font=\tiny] {H$_2$O};
                    \draw[<->] (0,2.7) -- (1.5,2.7) node[midway,above,font=\tiny] {150$\times$};
                \end{tikzpicture}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions \& Future Outlook}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{The PVDF Paradox}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{The Entrenched Incumbent}
                \textbf{Unmatched Advantages:}
                \begin{itemize}\small
                    \item \textbf{5.0 V electrochemical stability} (unique)
                    \item Thermal stability $>$400\si{\celsius}
                    \item Established supply chain
                    \item Proven reliability at scale
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{The Fatal Flaws}
                \textbf{Four Fundamental Problems:}
                \begin{enumerate}\small
                    \item \textbf{NMP toxicity} \& environmental cost
                    \item \textbf{Weak adhesion} (physical VdW only: 8.7-11 N/m)
                    \item \textbf{Si-anode incompatibility} (brittle vs. 300\% strain)
                    \item \textbf{HF recycling hazard} \& non-renewable feedstock
                \end{enumerate}
            \end{alertblock}
        \end{column}
    \end{columns}
    
    \vspace{0.3cm}
    \begin{center}
        \Large\textbf{PVDF is simultaneously essential and obsolescent}
    \end{center}
\end{frame}

\begin{frame}{Future Research Outlook: Three Paths Forward}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Path 1: "Greening" PVDF}
                \footnotesize \textbf{Preserve performance, eliminate NMP}
                \begin{itemize}\footnotesize
                    \item HSP-guided solvent screening
                    \item Cyrene, GVL (RED $<$ 1.0)
                    \item Bio-derived, non-toxic alternatives
                    \item SC-CO$_2$ recycling with DMSO
                \end{itemize}
            \end{block}
            
            \vspace{0.1cm}
            \begin{block}{Path 2: "Replacing" PVDF}
                \footnotesize \textbf{Develop functional replacements}
                \begin{itemize}\footnotesize
                    \item PAA/Alginate for Si-anodes
                    \item Chemical H-bonding (78.3 N/m)
                    \item Self-healing networks
                    \item Water-based processing
                \end{itemize}
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{block}{Path 3: "Evolving" PVDF}
                \footnotesize \textbf{Leverage unique properties}
                \begin{itemize}\footnotesize
                    \item Gel-polymer electrolytes (PVDF-HFP)
                    \item Composite solid-state electrolytes
                    \item High-voltage cathode binder
                    \item Functional copolymers
                \end{itemize}
            \end{block}
            
            \vspace{0.1cm}
            \begin{exampleblock}{Strategic Direction}
                \footnotesize \textbf{Bifurcated market emerging:}
                \begin{itemize}\footnotesize
                    \item PVDF: High-voltage cathodes (5V)
                    \item Aqueous: Anodes (graphite, Si)
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Key Takeaways}
    \begin{enumerate}\small
        \item \textbf{Crystallinity is master property:} 32\% $\rightarrow$ 9$\times$ adhesion, 82\% vs. 64\% retention
        \item \textbf{NMP drives costs:} 50-150$\times$ more than water, plus infrastructure
        \item \textbf{Aqueous binders stronger:} PAA $\sim$2 N/cm vs. PVDF $\sim$1 N/cm
        \item \textbf{PVDF fails next-gen:} Too brittle for Si (300\% expansion), unstable in Na-ion
        \item \textbf{Application-specific future:} Aqueous for anodes, PVDF for high-voltage
        \item \textbf{Market booming:} \$5.99B (2024) $\rightarrow$ \$28.75B (2033), 20.3\% CAGR
    \end{enumerate}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{References}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% --- REFERENCES ---
\begin{frame}[allowframebreaks]{References}
    \bibliography{../references}
\end{frame}

% --- FINAL SLIDE ---
\begin{frame}[plain]
    \vfill
    \begin{center}
        {\Huge \textcolor{DarkNavy}{Thank You}}
        
        \vspace{1cm}
        
        {\Large Questions and Discussion}
        
        \vspace{1cm}
    \end{center}
    \vfill
\end{frame}

\end{document}
